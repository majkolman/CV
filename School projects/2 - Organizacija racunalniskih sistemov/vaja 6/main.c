...
int main(void)
{
  ...
  HAL_DMA_Start(&dma1_struct, (uint32_t)source, (uint32_t)destination, SIZE);
  // čakamo dokler se prenos ne konča

  start_timer();
  HAL_DMA_PollForTransfer(&dma1_struct, HAL_DMA_FULL_TRANSFER, HAL_MAX_DELAY);
  stop_timer();
  cas_DMA = get_time(); //cas 32 je 7519, 50 je 11812
  ...
  stop_timer();
  cas_LOOP = get_time(); //cas 32 je 16536, 50 je 25771
