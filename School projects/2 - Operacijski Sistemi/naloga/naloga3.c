...
void sigchld_handler(int signum){
    int pidd, status, serrno;
    serrno = errno;
    while(pidd = waitpid(-1, &status, WNOHANG) > 0){
...
void run_tokenize(char* line){
    ...
    if(!printed && debug_level)printf("Input line: '%s'\n",line);
...
void redirect(char* cmd){
    ...
    if(pidd < 0){
        ...
        if(vhod_f){
            int fdin = open(vhod, O_RDONLY);
            dup2(fdin, 0);
            close(fdin);
        }
        if(izhod_f){
            int fdout = open(izhod, O_WRONLY | O_CREAT | O_TRUNC, 0644);
            dup2(fdout ,1);
            close(fdout);
        }
        find_builtin(cmd);
        ...
        int signal;
        waitpid(pidd, &signal, 0);
        if(WIFEXITED(signal)) signal = WEXITSTATUS(signal);
        return_status = signal;
    }
}
int find_builtin(char* cmd){
...
int execute_external(char* ime){
    ...
    if(mypid < 0){
        ...
        if(!background){
            waitpid(mypid, &last_status, 0);
            if(WIFEXITED(last_status)) last_status = WEXITSTATUS(last_status);
        }
    }
    return_status = last_status;
}
int debug(char* arg){
    if(arg == NULL){
        printf("%d\n",debug_level);
...
int waitone(char* pidd){
    ...
    int intpid;
    if(pidd != NULL){
        //preverimo ce obstaja otrok z pid
        intpid = atoi(pidd);
    }else{
        //najdi prvega otroka
        intpid = -1;
